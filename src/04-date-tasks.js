...
function isLeapYear(date) {
  ...
  else if (date.getFullYear() % 100 === 0) leap = false;
  else if (date.getFullYear() % 4 === 0) leap = true;
  else leap = false;
  return leap;
}
...
function timeSpanToString(startDate, endDate) {
  function n0(value, count) {
    let x = value;
    x = value.toString();
    return '0'.repeat(count - x.length) + x;
  }
...
function angleBetweenClockHands(date) {
  const h = new Date(date).getHours() - 3;
  const m = new Date(date).getMinutes();
  if (Math.abs(((((m / 60) + (h % 12)) / 12) - (m / 60)) * 360) > 180) {
    return ((Math.round((360 - Math.abs(((((h % 12) + (m / 60)) / 12)
    - (m / 60)) * 360)) * 10) / 10) * Math.PI) / 180;
  }
  return ((Math.round((Math.abs(((((h % 12) + (m / 60)) / 12)
  - (m / 60)) * 360)) * 10) / 10) * Math.PI) / 180;
}
